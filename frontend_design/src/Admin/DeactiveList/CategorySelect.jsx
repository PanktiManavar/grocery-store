...
const CategorySelect = () => {
  ...
  const getCategory = async () => {
    setLoading(true);
    let result = await fetch('api/categoryDeactiveselect');
  ...
  const deleteCategory = async (id, e) => {
    ...
    if (result) {
      swal({
        ...
        text: "Your Category is Activated!",
        icon: "success",
      });
      getCategory();
    }
                  ...
                  <tbody>
                    {
                      category.length > 0 ? category.map((item, index) => (
                        <tr key={item._id}>
                          ...
                          <td>{item.cname}</td>
                          <td>
                            <BsArrowDownCircleFill className="" onClick={(e) => deleteCategory(item._id, e)} style={{ padding: 2, fontSize: 26 }} />
